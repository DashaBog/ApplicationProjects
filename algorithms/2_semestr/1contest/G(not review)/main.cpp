...
class AdjacencyListGraph : public AbstructGraph<Vertex, Edge> {
 public:
  AdjacencyListGraph(size_t vertices_number, const std::vector<Edge>& edges)
    ...
    for (size_t edge = 0; edge < edges.size(); ++edge) {
      ...
      multiplicityedges_[edges[edge].Start()][edges[edge].Finish()]++;
      multiplicityedges_[edges[edge].Finish()][edges[edge].Start()]++;
    }
  }

  void DfsBridges(int vertex, int p, std::vector<Vertex>& used, int& timer,
                  std::vector<Vertex>& briges) {
    used[vertex] = 1;
    tin_[vertex] = timer;
    ret_[vertex] = timer;
    ++timer;
    for (size_t i = 0; i < list_[vertex].size(); ++i) {
      ...
      int numberto = list_[vertex][i].second;
      if (to == p) {
        continue;
      }
      if (used[to] == 1) {
        ...
        if (ret_[to] > tin_[vertex] && (multiplicityedges_[to][vertex] <= 1) &&
            (multiplicityedges_[vertex][to] <= 1)) {
          briges.push_back(++numberto);
        }
      }
    }
  }
 ...
 private:
  ...
  std::unordered_map<Vertex, std::unordered_map<Vertex, Vertex>>
      multiplicityedges_;
